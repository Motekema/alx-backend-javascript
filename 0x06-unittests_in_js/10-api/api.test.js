...
describe('API integration test', () => {
  ...
  // Test case for verifying the response of the POST /login endpoint
  it('POST /login returns valid response', (done) => {
    // Making a POST request to the /login endpoint with a JSON body
  ...
  // Test case for verifying the response of the GET /available_payments endpoint
  it('GET /available_payments returns valid response', (done) => {
    // Making a GET request to the /available_payments endpoint
    request.get(`${API_URL}/available_payments`, (_err, res, body) => {
      // Assertion to check if the status code is 200
      expect(res.statusCode).to.be.equal(200);
      // Assertion to check if the response body is the expected JSON object
